...
# option.add_argument(f'{PROXY}')
# option.add_argument('--proxy-server={}'.format(PROXY))
# option = {'proxy': {'http://youparcel:Proxy2022!@us-ca.proxymesh.com:31280'} }
# option.add_argument(f'--proxy-server=%s' % 'Proxy2022!@de.proxymesh.com' + ":" + '31280')
...
for name in (driver.find_elements(By.XPATH,'//a[@class="MuiTypography-root MuiLink-root MuiLink-underlineNone MuiTypography-colorPrimary"]/h3')):
    ...
    companies_data.append(json_data)
    for contact in (driver.find_elements(By.XPATH,'//span[@class="MuiButton-label"]/preceding::button[@class="MuiButtonBase-root MuiButton-root MuiButton-text MuiButton-textPrimary MuiButton-fullWidth"]')):
        ...
        companies_data.append(json_data)
        for adress in (driver.find_elements(By.XPATH,'//div[@class="Box__Div-dws99b-0 gFKsYt"]')):
            json_data['adress'] = adress.text
            companies_data.append(json_data)
            with open('JSON_{}.json'.format(pg), 'w') as fp:
                json.dump(companies_data, fp)
